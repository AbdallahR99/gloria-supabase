...
export async function handleGetBundleBySlug(req, supabase) {
  ...
  const { data: product, error: productError } = await supabase.from("products").select("id").or(`slug.eq.${slug},slug_ar.eq.${slug}`).single();
  if (productError || !product) {
    throw productError ?? new Error("Product not found");
  }
  const { data: bundles, error: bundleError } = await supabase.from("bundles").select(`
      id,
      product_id,
      is_active,
    ...
    `).eq("product_id", product.id).eq("is_active", true).maybeSingle();
  ...
  if (!authError && user) {
    ...
    const { data: cartItems, error: cartError } = await supabase.from("cart").select("product_id").eq("user_id", user.id).eq("is_deleted", false).in("product_id", productIds);
  ...
  bundles.old_price = childProducts.reduce((sum, p)=>sum + (p.old_price ?? p.price ?? 0), 0);
